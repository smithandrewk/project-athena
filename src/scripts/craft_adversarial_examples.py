...
def generate_ae(model, data, labels, attack_configs, eot=False, save=False, output_dir=None):
    ...
    for index, id in enumerate(range(num_attacks)):
        print("STARTING NEW ATTACK") #AS
        key = "configs{}".format(id)
        attack_args = attack_configs.get(key)
        attack_args["eot"] = eot
        print(f"Generating: {id}: {index}/{data.shape[0]}")
...
if __name__ == '__main__':
    ...
    # subsamples, sublabels = subsampling(data=bs,labels=labels,num_classes=10,ratio=0.05,filepath="Task2/results",filename="mnist")
    # Normal approach
