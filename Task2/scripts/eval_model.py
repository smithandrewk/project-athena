...
def evaluate_cnn(trans_configs, model_configs,
    ...
    FGSM_rotation = [ae_list[0],ae_list[2],ae_list[4]]
    FGSM_rotation_UM = [UM[0],UM[2],UM[4]]
    FGSM_rotation_e = [e[0],e[2],e[4]]
    FGSM_rotation_PGDADT = [PGDADT[0],PGDADT[2],PGDADT[4]]
    FGSM_translation = [ae_list[1],ae_list[3],ae_list[5]]
    FGSM_translation_UM = [UM[1],UM[3],UM[5]]
    FGSM_translation_e = [e[1],e[3],e[5]]
    FGSM_translation_PGDADT = [PGDADT[1],PGDADT[3],PGDADT[5]]
    PGD_rotation = [ae_list[6],ae_list[8],ae_list[10]]
    PGD_rotation_UM = [UM[6],UM[8],UM[10]]
    PGD_rotation_e = [e[6],e[8],e[10]]
    PGD_rotation_PGDADT = [PGDADT[6],PGDADT[8],PGDADT[10]]
    PGD_translation = [ae_list[7],ae_list[9],ae_list[11]]
    PGD_translation_UM = [UM[7],UM[9],UM[11]]
    PGD_translation_e = [e[7],e[9],e[11]]
    PGD_translation_PGDADT = [PGDADT[7],PGDADT[9],PGDADT[11]]
    ##
    data = {
        "UM": FGSM_rotation_UM,
    ...
    ##
    data = {
        "UM": FGSM_translation_UM,
        "Ensemble": FGSM_translation_e,
        "PGD-ADT": FGSM_translation_PGDADT
    }

    fig, ax = plt.subplots()
    bar_plot(ax, data, total_width=.8, single_width=.9)
    plt.xticks(range(3),FGSM_translation)
    plt.ylabel("Error")
    plt.gcf().set_size_inches(15, 10)
    plt.savefig('FGSM_translation.jpg',dpi=100,bbox_inches='tight')
    plt.show()
    ##3
    data = {
        "UM": PGD_rotation_UM,
    ...
    ##
    data = {
        "UM": PGD_translation_UM,
        "Ensemble": PGD_translation_e,
        "PGD-ADT": PGD_translation_PGDADT
    }

    fig, ax = plt.subplots()
    bar_plot(ax, data, total_width=.8, single_width=.9)
    plt.xticks(range(3),PGD_translation)
    plt.ylabel("Error")
    plt.gcf().set_size_inches(15, 10)
    plt.savefig('PGD_translation.jpg',dpi=100,bbox_inches='tight')
    plt.show()
